...
/**
 * Resolve credential for a provider. Priority:
 * 1. Environment variable (if envKey is provided)
 * 2. OS keyring (keytar)
 * 3. File fallback (~/.config/absolute/auth/credentials.json)
 */
export async function resolveCredential(providerId: string, envKey?: string): Promise<string | null> {
  ...
  if (keytar) {
    try {
      const key = await keytar.getPassword('absolute', providerId);
      if (key) return key;
    } catch {
  ...
  return loadCredentialsFile().find((c) => c.provider === providerId)?.key ?? null;
}
