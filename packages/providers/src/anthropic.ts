...
export class AnthropicProvider {
  ...
  async isConfigured(): Promise<boolean> {
    ...
    return (await resolveCredential(this.id, this.envKey)) !== null;
  }

  private async resolveApiKey(): Promise<string> {
    if (this.apiKeyOverride) return this.apiKeyOverride;
    const apiKey = await resolveCredential(this.id, this.envKey);
    if (!apiKey) {
      throw new Error(
        'Anthropic is not configured. Set ANTHROPIC_API_KEY or run `absolute provider set anthropic <key>`.'
      );
    }
    return apiKey;
  }
