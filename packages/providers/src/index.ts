...
export class ProviderManager {
  ...
  /** Find first configured provider (for auto-detection). */
  firstConfigured(): LLMProvider | undefined {
    return [...this.providers.values()].find(
      (p) => p.isConfigured() as unknown as boolean
    );
  }
  ...
  async test(id: string): Promise<ProviderTestResult> {
    ...
    if (!(await provider.isConfigured())) {
      return {
        ok: false,
        message: `${provider.name} has no API key stored. Set ${provider.envKey ?? 'the API key'} or run \`absolute provider set ${id} <key>\`.`,
      };
    }
...
export { storeCredential, getCredential, resolveCredential, deleteCredential } from './credentials.js';
