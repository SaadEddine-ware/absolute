...
export interface SyncResult {
  ...
  /** Cosine similarity between prompt and active goal vectors (0-1). */
  similarity: number;
  /** Current adaptive threshold used for the decision. */
  threshold: number;
...
export async function detectSessionContext(
  ...
  const sessionContext = loadSessionContext(db.db, sessionId, {
    mode: 'prompt',
    maxTokens,
  });
  try {
    ...
    return {
      ...
      threshold: getUserSettings(db.db).similarity_threshold,
      goal,
      goalCreated,
      goalAction,
    };
    ...
    return { sessionContext, relevantMemories: [], note: undefined, decision: 'continue', similarity: 0, threshold: 0.6, goal: getActiveGoals(db.db, sessionId)[0] ?? null };
  }
}
