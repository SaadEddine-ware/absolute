...
const REQUIRED_TOKEN_KEYS = [
  'primary', 'accent', 'user', 'assistant', 'system', 'text', 'muted',
  'border', 'bg', 'statusOk', 'statusWarn', 'statusErr', 'header',
] as Array<keyof ThemeVariant>;
...
function isVariant(v: unknown): v is ThemeVariant {
  ...
  if (!REQUIRED_TOKEN_KEYS.every((k) => typeof o[k] === 'string')) return false;
  // Fill defaults for new tokens so old theme files still work
  for (const [key, val] of Object.entries(NEW_TOKEN_DEFAULTS)) {
    if (typeof o[key] !== 'string') o[key] = val;
  }
  return true;
}
