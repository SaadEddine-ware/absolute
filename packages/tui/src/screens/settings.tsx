    ...
    <Box flexDirection="column" height={rows} paddingX={1}>
      ...
      <Box flexDirection="column" flexGrow={1} overflowY="hidden">
        ...
        <Text color={theme.text}>
          {'  '}provider <Text color={theme.muted}>{info.config.provider ?? 'openai'}</Text>
          {info.config.model ? (
            <Text color={theme.muted}>{' · '}{info.config.model}</Text>
          ) : null}
        </Text>
        ...
        <Text color={theme.text}>
          {'  '}value <Text color={theme.muted}>{settingsRow.similarity_threshold.toFixed(2)}</Text>
