  ...
  return (
    <Box flexDirection="column" height={rows} paddingX={1}>
      ...
      <Box flexDirection="column" flexGrow={1} overflowY="hidden">
        {flat.length === 0 ? (
          <Text color={theme.muted} dimColor>
            {'  No memories yet. Chat to create them.'}
          </Text>
        ) : (
          visible.map((row, i) => {
            const idx = start + i;
            ...
            return (
              <Box key={m.id}>
                <Text color={selectedRow ? theme.primary : theme.border}>
                  {selectedRow ? '\u203a ' : '  '}
                </Text>
                <Text color={theme.border}>{'  '.repeat(row.depth)}</Text>
                <Text color={selectedRow ? theme.primary : theme.muted}>
                  {marker}{' '}
                </Text>
                <Text color={theme.muted}>{`[${m.type}]`}</Text>
                <Text color={theme.muted} dimColor>
                  {' '}
                  {m.importance}
                  {' '}
      ...
      {selected && (
        <Box borderStyle="single" borderColor={theme.border} paddingX={1} marginBottom={1}>
          ...
          <Text color={theme.muted} dimColor>
            {'  '}
            {selected.id} · {selected.keys || '{}'} · {selected.created_at.slice(0, 19)}
          </Text>
        </Box>
      )}
