...
import { recordSwitchFeedback as recordSwitchFeedbackCore, updateSessionTokensUsed, estimateTokens, type SystemPromptConfig } from '@absolute/core';
...
export interface UseChatOptions {
  ...
  /** Called after each SYNC detection with the real-time context engine values. */
  onDetection?: (result: {
    similarity: number;
    threshold: number;
  ...
  /** Token budget for session memories (from config memory.maxTokensPerSession). */
  maxTokens?: number;
  /** System prompt customization from config.json. */
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      if (options.db && options.provider && sessionId && sessionId !== 'none') {
        ...
        const sync = await detectSessionContext(db, provider, trimmed, sessionId, options.maxTokens);

        // Push real detection values to the context engine panel.
        options.onDetection?.({
          similarity: sync.similarity,
          threshold: sync.threshold,
          decision: sync.decision,
          goal: sync.goal ?? null,
          goalEvaluated: sync.similarity > 0,
        });
        ...
        // Track total tokens used in this session (system prompt + history + user).
        const totalChars = llmMessages.reduce((sum, m) => sum + m.content.length, 0);
        const tokensUsed = estimateTokens(' '.repeat(totalChars));
        try {
          updateSessionTokensUsed(db.db, sessionId, tokensUsed);
        } catch {
          // best-effort
        }
      }
    ...
    [responder, getContext, options.db, options.provider, options.onExchange, options.onDetection, options.maxTokens, options.promptConfig, mode, askConfirm, pushSystem]
  );
